...
function addEnvClean(env) {
    return new Promise((resolve, reject) => {
        console.log(`Añadiendo ${env.key} con valor limpio...`);
        const child = spawn('npx', ['vercel', 'env', 'add', env.key, 'production'], {
            shell: true
        });

        // Enviar SOLAMENTE el valor de la variable, sin respuestas interactivas y sin saltos de línea adicionales
        ...
        child.on('close', (code) => {
            if (code === 0) {
                console.log(`[ÉXITO] ${env.key} agregada con éxito.`);
                resolve();
            } else {
...
async function run() {
    // 1. Eliminar variables anteriores con valores corruptos
    for (const env of envs) {
        deleteEnv(env.key);
    }

    // 2. Agregar nuevas variables con valores limpios
    for (const env of envs) {
        try {
            await addEnvClean(env);
        } catch (e) {
            // Ignorar para continuar
        }
    }
    console.log('Actualización limpia de variables en Vercel finalizada.');
}
